...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeVirtualpaymentAmount = BitcoinUnits::formatWithUnit(nDisplayUnit, nAnonymizeVirtualpaymentAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeVirtualpaymentAmount = strAnonymizeVirtualpaymentAmount.remove(strAnonymizeVirtualpaymentAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVirtualpaymentAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeVirtualpaymentAmount*COIN) nMaxToAnonymize = nAnonymizeVirtualpaymentAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeVirtualpaymentAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeVirtualpaymentAmount));
        strAnonymizeVirtualpaymentAmount = strAnonymizeVirtualpaymentAmount.remove(strAnonymizeVirtualpaymentAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVirtualpaymentAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeVirtualpaymentAmount == 0){
