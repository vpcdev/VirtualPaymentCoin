...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeVirtualpaymentAmount"))
        settings.setValue("nAnonymizeVirtualpaymentAmount", 1000);
    nAnonymizeVirtualpaymentAmount = settings.value("nAnonymizeVirtualpaymentAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeVirtualpaymentAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeVirtualpaymentAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeVirtualpaymentAmount:
            return QVariant(nAnonymizeVirtualpaymentAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeVirtualpaymentAmount:
            nAnonymizeVirtualpaymentAmount = value.toInt();
            settings.setValue("nAnonymizeVirtualpaymentAmount", nAnonymizeVirtualpaymentAmount);
            emit AnonymizeVirtualpaymentAmountChanged(nAnonymizeVirtualpaymentAmount);
            break;
        default:
            break;
        }
