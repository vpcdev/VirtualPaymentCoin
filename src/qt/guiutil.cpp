...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("virtualpayment"))
        return false;
...
{
    // Convert virtualpayment:// to virtualpayment:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("virtualpayment://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "virtualpayment:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Virtualpayment.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Virtualpayment.lnk
...
{
    return GetAutostartDir() / "virtualpayment.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a virtualpayment.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Virtualpayment\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the virtualpayment app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add virtualpayment app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Virtualpayment-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  virtualpayment-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Virtualpayment-Qt"));
