...
void Shutdown()
{
    ...
    RenameThread("virtualpayment-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: virtualpayment.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: virtualpaymentd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizevirtualpaymentamount=<n> " + _("Keep N Virtualpayment anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Virtualpayment is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Virtualpayment is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Virtualpayment version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Virtualpayment server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Virtualpayment") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Virtualpayment to complete") << "\n";
    ...
    nAnonymizeVirtualpaymentAmount = GetArg("-anonymizevirtualpaymentamount", 0);
    if(nAnonymizeVirtualpaymentAmount > 999999) nAnonymizeVirtualpaymentAmount = 999999;
    if(nAnonymizeVirtualpaymentAmount < 2) nAnonymizeVirtualpaymentAmount = 2;
    ...
    LogPrintf("Anonymize Virtualpayment Amount %d\n", nAnonymizeVirtualpaymentAmount);
