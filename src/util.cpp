...
int nAnonymizeVirtualpaymentAmount = 1000;
...
#else
    const char* pszModule = "virtualpayment";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Virtualpayment
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Virtualpayment
    // Mac: ~/Library/Application Support/Virtualpayment
    // Unix: ~/.virtualpayment
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Virtualpayment";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "virtualpayment.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "virtualpaymentd.pid"));
